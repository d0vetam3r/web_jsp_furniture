...
public class Notice_dao {
	...
	public int getTotalCount(String select, String search) {
		int result = 0;
		String query = "select count(*) from homepage_김세훈_notice where " + select + " like '%" + search + "%'";
		try {
			con = DBConnection.getConnection();
			ps = con.prepareStatement(query);
			rs = ps.executeQuery();
	...
	public int noticeUpdate(String title, String content, String reg_date, String no) {
		int result = 0;
		String query = "update homepage_김세훈_notice set title='" + title + "',content='" + content + "',reg_date='"
	...
	public int updateHit(String hit, String no) {
		...
		temp++;
		String query = "update homepage_김세훈_notice set hit = '" + temp + "' where no = '" + no + "'";
		try {
			con = DBConnection.getConnection();
			ps = con.prepareStatement(query);
			result = ps.executeUpdate();
	...
	public Notice_dto getView(String no) {
		...
		String query = "select no, title, content, attach, reg_id,to_char(reg_date,'yyyy-MM-dd') as reg_date, hit\r\n"
				+ "from homepage_김세훈_notice where no = '" + no + "'";
		String hit = "";
		try {
			con = DBConnection.getConnection();
			ps = con.prepareStatement(query);
			...
			if (rs.next()) {
				...
				dto = new Notice_dto(no, title, content, attach, hit, reg_info, reg_date);
			}
			if (dto != null) {
	...
	public ArrayList<Notice_dto> getNoticeList(String select, String search) {
		...
		;
		String query = "select no,title,content,attach,reg_id,to_char(reg_date,'yyyy-MM-dd') as reg_date,hit from homepage_김세훈_notice\r\n"
		...
		try {
			...
			while (rs.next()) {
				...
				Notice_dto dto = new Notice_dto(no, title, content, attach, hit, reg_info, reg_date);
				dtos.add(dto);
	...
	// 전체목록(페이징)
	public ArrayList<Notice_dto> getNoticeListPaging(String select, String search, int start, int end) {
		ArrayList<Notice_dto> dtos = new ArrayList<>();
		;
		String query = "select * from(select tbl.*, rownum as rnum from(select no,title,content,attach,reg_id,to_char(reg_date,'yyyy-MM-dd')\r\n"
				+ "as reg_date,hit from homepage_김세훈_notice where " + select + " like '%" + search
	...
	public String getMaxNo() {
		String result = null;
		String query = "select nvl(max(no),'n000') from homepage_김세훈_notice";
		try {
			con = DBConnection.getConnection();
			ps = con.prepareStatement(query);
			rs = ps.executeQuery();
